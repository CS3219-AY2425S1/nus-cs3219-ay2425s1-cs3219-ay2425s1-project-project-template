...
BASE_TAG="delishad21"
VERSION_BASE="prod1.0."

# Check if a tag argument is provided
if [ -z "$1" ]; then
    echo "Error: No tag argument provided. Usage: ./push_production.sh <tag>"
    exit 1
fi

# Assign the tag argument (e.g., 'prod', 'staging') to a variable
TAG=$1
...
get_latest_version() {
    ...
    if [ -z "$latest_version" ]; then
        ...
        # Extract the X part of the version (prod1.0.X)
        echo "${latest_version##*.}"
    fi
}
...
for service_name in "${!services[@]}"; do
    ...
    # Tag for the new image with version number (e.g., prod1.0.X)
    new_tag="$BASE_TAG/$service_name:$VERSION_BASE$new_version"

    # Tag for the custom environment (e.g., prod, staging, etc.)
    custom_tag="$BASE_TAG/$service_name:$TAG"

    echo "Building $service_name from directory $service_dir with tags $new_tag and $custom_tag..."

    # Build the Docker image
    docker build -t "$new_tag" "$service_dir"

    # Tag the same image with the custom tag (e.g., prod or staging)
    docker tag "$new_tag" "$custom_tag"

    # Push both the versioned image and the custom tag in parallel
    (
        docker push "$new_tag" && \
        echo "Successfully pushed $service_name as $new_tag" && \
        docker push "$custom_tag" && \
        echo "Successfully pushed $service_name as $custom_tag"
    ) &
done
