...
;(async () => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('requestMatch', async (data: any) => {
            ...
            logger.info(`User ${userId} has requested for a match with difficulty ${difficulty}, language ${language} and categories ${categories}`, { service: 'matching-service', timestamp: new Date().toISOString() })
