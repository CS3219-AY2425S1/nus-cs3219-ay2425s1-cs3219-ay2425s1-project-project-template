...
const MONGO_URI_MS = process.env.MONGO_URI_MS ;

const app = express();
app.use(cors({ origin: FRONTEND_URL, credentials: true }));
app.use(express.json());

mongoose.connect(MONGO_URI_MS!).then(() => console.log("Connected to MongoDB"))
...
io.on("connection", (socket) => {
  ...
  authenticateAccessToken(accessToken).then((user) => {
