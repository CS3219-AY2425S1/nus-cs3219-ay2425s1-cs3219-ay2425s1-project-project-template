#!/bin/bash
envsubst '\$USER_SERVICE_ADDR \$QUESTION_SERVICE_ADDR \$MATCH_NOTIFICATION_SERVICE_ADDR \$COLLABORATION_SERVICE_ADDR \$DOMAIN_NAME'< /tmp/default.conf > /etc/nginx/conf.d/default.conf
