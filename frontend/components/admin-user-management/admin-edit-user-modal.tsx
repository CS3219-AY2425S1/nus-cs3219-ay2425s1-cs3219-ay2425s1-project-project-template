...
const AdminEditUserModal: React.FC<AdminEditUserModalProps> = ({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const response = await updateUser(auth.token, editingUser.id, editingUser?.username, editingUser?.email);
    if (!response.ok) {
        toast({
          title: "Unknown Error",
          ...
          <DialogContent>
            ...
            <DialogFooter>
              <Button
                ...
                disabled={!auth?.token || !editingUser?.id || (!editingUser?.email && !editingUser?.username)}
              >
                Save changes
              </Button>
